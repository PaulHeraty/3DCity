...
#define USE_FXAA311
#define FXAA_INTEL_GATHER_ISSUE_WORKAROUND
...
// Original FXAA Shader
#ifndef USE_FXAA311
void main()
{
#if 0
   vec4 testpl = texture2D( s_texture, v_texCoord);
   testpl.r = 1.0f;
   gl_FragColor = testpl;
//  gl_FragColor = vec4(1.0,1.0,0.0,0.5);
#else
  vec2 u_texCoordOffset = vec2(1.0/1920.0, 1.0/720.0);
...
// Otherwise use 3.11 FXAA shader
#else
/*============================================================================
...
============================================================================*/
    ...
    //FxaaFloat FxaaLuma(FxaaFloat4 rgba) { return dot(sqrt(rgba.xyz), vec3(0.299, 0.587, 0.114)); }
#else
    FxaaFloat FxaaLuma(FxaaFloat4 rgba) { return rgba.y; }
    //FxaaFloat FxaaLuma(FxaaFloat4 rgba) { return rgba.b; }
    //FxaaFloat FxaaLuma(FxaaFloat4 rgba) { return dot(sqrt(rgba.xyz), vec3(0.299, 0.587, 0.114)); }
#endif    
...
/*--------------------------------------------------------------------------*/
    ...
    posM.y = pos.y;
        ...
        #if (FXAA_GREEN_AS_LUMA == 0)
            //#define lumaM rgbyM.w
            FxaaFloat lumaPH = FxaaLuma(rgbyM);
            #define lumaM lumaPH;
        #else
            #define lumaM rgbyM.y
        #endif

        FxaaFloat lumaM = FxaaLuma(FxaaTexOff(tex, posM, FxaaInt2( 0, 0), fxaaQualityRcpFrame.xy));
...
void main()
{
    // Resolution constant for now
    vec2 g_Resolution = vec2(1920.0, 720.0);
    // This used to be the FXAA_QUALITY__SUBPIX define.
    // It is here now to allow easier tuning.
    // Choose the amount of sub-pixel aliasing removal.
    // This can effect sharpness.
    ...
    //   Tune by looking at mostly non-green content,
    //   then start at zero and increase until aliasing is a problem.
    float m_EdgeThresholdMin=0.0833;

    FxaaFloat2 _pos = v_texCoord;
    FxaaFloat4 fxaaConsolePosPos = vec4(0);
    //FxaaTex tex = s_texture;
    //FxaaTex fxaaConsole360TexExpBiasNegOne = s_texture;
    //FxaaTex fxaaConsole360TexExpBiasNegTwo = s_texture;
...
#ifdef FXAA_INTEL_GATHER_ISSUE_WORKAROUND
    _pos.xy = (_pos.xy + vec2(0.001, 0.001) / g_Resolution );
#endif
    ...
    outcolor.r =  dot(sqrt(outcolor.rgb), vec3(0.299, 0.587, 0.114)); 
